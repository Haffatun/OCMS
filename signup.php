...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password) || empty($name)) {
        ...
        if ($stmt->fetchColumn() > 0) {
            ...
            try {
                // Check max student user_id
                $stmt = $pdo->query("SELECT MAX(user_id) FROM users WHERE role = 'student'");
                $last_student_id = $stmt->fetchColumn();

                if (!$last_student_id) {
                    // No student exists yet, assign 10001 as user_id
                    $new_user_id = 10001;
                    $stmt = $pdo->prepare("INSERT INTO users (user_id, email, password_hash, name, role, is_active) VALUES (?, ?, ?, ?, 'student', TRUE)");
                    $stmt->execute([$new_user_id, $email, $password_hash, $name]);
                    $_SESSION['user_id'] = $new_user_id;
                } else {
                    // Insert normally letting auto_increment handle user_id
                    $stmt = $pdo->prepare("INSERT INTO users (email, password_hash, name, role, is_active) VALUES (?, ?, ?, 'student', TRUE)");
                    $stmt->execute([$email, $password_hash, $name]);
                    $_SESSION['user_id'] = $pdo->lastInsertId();
                }

                $_SESSION['role'] = 'student';
                header("Location: index.php"); // Redirect to home/dashboard after signup
                exit;
...
<body>
    ...
    <main>
        <section class="signup-container">
            ...
            <!-- PHP error message -->
            <!-- Replace below with PHP echo if needed -->
